...
namespace keccak
{
    public static class Gost_34_11_2012_safe
    {
        /// <summary>
        /// Вычисляет хэш по ГОСТ 34.11-2012
        /// </summary>
        /// <param name="data">Данные в двоичном виде</param>
        ...
        // <returns></returns>
        public static byte[] getHash512(byte[] data, bool reversed)
        {
            if (!reversed)
            {
        ...
        public static byte[] getHash256(byte[] data, bool reversed)
        {
            byte[] h;
        ...
        public static readonly byte[] TAU = {   0, 8, 16, 24, 32, 40, 48, 56, 1, 9, 17, 25, 33, 41, 49, 57, 2, 10, 18, 26, 34, 42, 50, 58, 
        ...
        /// <returns></returns>
        public static byte[] P(byte[] data512, long start = 0)
        {
            var result = new byte[64];
            for (long i = 0; i < 64; i++)
            {
                result[i] = data512[TAU[i] + start];
            }
        ...
        public static byte[] L(byte[] data)
        {
            var result = new byte[data.LongLength];
            for (int i = 0; i < data.LongLength; i += 8)
            {
                ulong t = BytesToLong(data, i);
                t = l(t);
                LongToBytes(result, i, t);
            }

            return result;
        }

        private static void LongToBytes(byte[] result, int i, ulong t)
        {
            for (int j = 7; j >= 0; j--)
            {
        ...
        private static ulong BytesToLong(byte[] data, int i)
        {
            ulong result = 0;
            for (int j = 0; j < 8; j++)
            {
        ...
        public static byte[] X(byte[] k, byte[] a)
        {
            var result = new byte[k.LongLength > a.LongLength ? a.LongLength : k.LongLength];
        ...
        public static byte[] g(byte[] h, byte[] m, byte[] N)
        {
            return 
                    X
                    (
        ...
        public static byte[] E(byte[] K, byte[] m)
        {
            byte[][] k = new byte[14][];
            ...
            for (int i = 2; i <= 13; i++)
            {
                k[i] = L(P(S(X(k[i - 1], C[i - 1]))));
            }

            return Elps(k, m, 13);
        }

        public static byte[] Elps(byte[][] K, byte[] m, int i)
        {
            if (i > 1)
                return X(K[i], L(P(S(Elps(K, m, i - 1)))));
            else
                return X(K[i], /*L(P(S(X(K[1], m))))*/ m);
        }

        public static void add(byte[] a1, byte[] a2)
        {
            int A = 0, B = 0;
